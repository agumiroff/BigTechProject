...
func NewContainer(ctx context.Context, opts ...Option) (*Container, error) {
	...
	genericContainer, err := startContainer(ctx, cfg)
	if err != nil {
		return nil, errors.Errorf("failed to start app container: %v", err)
	}

	mappedPort, err := genericContainer.MappedPort(ctx, nat.Port(cfg.Port+"/tcp"))
	if err != nil {
		return nil, errors.Errorf("failed to map port genericContainer: %v", err)
	}

	host, err := genericContainer.Host(ctx)
	if err != nil {
		return nil, errors.Errorf("failed to get host from genericContainer: %v", err)
	}
...
func streamContainerLogs(ctx context.Context, container testcontainers.Container, output io.Writer) {
