...
func (a *App) initGRPCServer(ctx context.Context) error {
	...
	repo := a.diContainer.InvRepository(ctx)
	if repo == nil {
		return fmt.Errorf("failed to initialize repository")
	}

	// Инициализируем сервис
	service := a.diContainer.InvService()
	if service == nil {
		return fmt.Errorf("failed to initialize service")
	}

	// Получаем gRPC server implementation
	inventoryServer := a.diContainer.InventoryServiceServer()
	if inventoryServer == nil {
		return fmt.Errorf("failed to initialize inventory server")
	}

	// Регистрируем Inventory Service
	inventoryv1.RegisterInventoryServiceServer(a.grpcServer, inventoryServer)
